...
SAMPLE_SECTIONS: tuple[Section, ...] = (
    ...
    Section("voice-samples.md", (
        ...
        "四、拒绝与划界（律师侧）",
        # 「四点五、她主动说的」2026-08-17 被 A/B 摘掉 —— 它想解决的问题
        # 「五、试探」已经解决了，而它要 527 字符且机械指标全线偏负。
        # 语料存档在 design-notes.md。
        "七、被夸",
        "十、生气", "十一、委屈", "十二、开心",
    ...
    # 「试探」原来门控在 S1+，理由是「S0 她刚加上好友，不会主动扔钩子」。
    #
    # **A/B 推翻了这条。** S0 单独测（n=10）：放回之后具体性 20%→33%、
    # 机械问题 26→18、人设违规 1→0 —— 四道门里效应最大的一个。
    #
    # 漏掉的一半是：这一节是她**唯一的主动样本**。砍掉之后 S0 只剩
    # 反应型样本（日常/累/关心/拒绝/被夸/生气/委屈/开心），她就只会接话。
    # 而且逻辑上本来就该想到 —— 他们是怎么从 S0 走到 S1 的？靠她扔钩子。
    Section("voice-samples.md", ("五、试探（她的越界句）",), flatten_tables=True),

    # 小节自己的注：「S1 阶段几乎每次越界后都跟一条。S3 基本消失。」
    # S0 没有越界所以无从撤回，S3 不撤回**就是甜**。
    #
    # A/B 证实了这道门（S0，n=10）：放回之后**人设违规 0→3、兜底 0→1**，
    # 关组 σ=1.02（其他组都在 0.1–0.2，说明有局直接崩了）。
    # 原因大概是 S0 她还没有什么可撤回的，模型把「算了。」「没什么。」
    # 当成常规回避语用，读起来就是敷衍。
