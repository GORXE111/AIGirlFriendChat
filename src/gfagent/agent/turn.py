...
def instructions(
    *,
    ...
    parts += [
        ...
        "- 例：他说了很重的话 → `{\"难过\": 0.4}`；"
        "他提起一件只有他们俩知道的事 → `{\"委屈\": -0.3, \"开心\": 0.2}`",
        "- **大部分回合应该是空的 `{}`。** 普通的一来一回不改变任何人的情绪状态，"
        "只有真的戳到了才填。每轮都填等于情绪一直在漂，玩家反而感觉不到波动。",
        "- 单项绝对值不要超过 0.5。真要把她说崩需要**连着两轮**，"
        "一句话就崩会显得莫名其妙。",
        "- 最多两项。",
        "- ⚠️ **按他实际说的判，不要按你希望的剧情走向判。**"
        "他好好说话就别给她加负面情绪。",
    ]
