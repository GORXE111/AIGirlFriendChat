...
STAGE_OPTION_GUIDE: dict[str, str] = {
    ...
    "S3": (
        "**在一起了，正热。** 这里有两个坑，第二个是踩了才发现的。\n"
        "  坑一：写成「体贴的成年人在照顾她」——"
        "三条都是「你早点睡」「别硬撑」「我陪着你」。那是护工不是男朋友。\n"
        "  坑二（**实测踩了**）：躲开坑一之后全写成调情 ——"
        "打趣、耍赖、吃醋、直球。这些**也全是关系动作**，"
        "结果整局二十多条消息只有一件事，两个人都被困在里面。\n"
        "  **正解：三条里至少一条的着力点在「她刚说的那件事」上。**"
        "她说风扇坏了，你就接风扇；她说买了两个关东煮，你就问那两个。\n"
        "  热恋不是「只谈我们」，是**谈别的事的时候你在里面**。"
    ),
}
...
STAGE_MOVES: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    # ⚠️ S3 原来三条全是**关系动作**（直球／耍赖／吃醋）。基准实测：
    # S3 的具体性 12–22%（其他阶段 20–59%）、话题跨度 2.5（其他 3–7），
    # 是四个阶段里最弱的一格 —— 但评审分最高，因为它读起来「很热恋」。
    #
    # 机制是个死循环：她只说关系 → 选项只能是关系动作 → 她下一轮
    # 只有关系上下文 → 重复。真实对局里玩家连着三轮打同一个动作，
    # 因为她没给别的东西可接。
    #
    # 所以「守住」改成**接住她说的那件事**。三条里至少要有一条的着力点
    # 在她的世界上，而不在两个人身上。
    "S3": (
        ("往前", "直球 —— 答应她、说想见她、给承诺"),
        ("守住", "接住她刚说的那件具体的事 —— 追问细节、拿它开个玩笑、"
                 "或者说一件你自己的对应的事。**着力点在事上，不在你俩身上**"),
...
def instructions(
    *,
    ...
    parts = [
        ...
        *(
            [
                "- ⚠️ **S3 的陷阱：把「热恋」写成「她只关心恋爱」。**"
                "基准实测 S3 是四个阶段里具体性最低、话题最窄的一格 ——"
                "她不说自己的事了，整局只有「你什么时候来」。\n"
                "  **一个只关心你的人是不迷人的。**"
                "她该说的还是风扇、练琴、她妈、那只猫 ——"
                "只是现在这些事里有他（「买了两个关东煮。」）。\n"
                "  见语气样本「九、S3」第一组。直球（你过来／想见你）"
                "**一次对话最多一条**，多了就不是她了。"
            ]
            if stage == "S3" else []
        ),
